...
SELECT Date, SB, Winner, Loser, MVP FROM superbowls 
ORDER BY SB;

-- count of records--
SELECT COUNT(*) FROM superbowls;
SELECT COUNT(Date) FROM superbowls;
SELECT COUNT(DISTINCT(State)) FROM superbowls;


-- What are the highest/lowest winner and losing points --
SELECT MIN(Winner_pts) AS "Lowest Winner Pts",
MIN(Loser_Pts)  AS "Lowest Loser Pts", 
MAX(Winner_pts)  AS "Highest Winner Pts", 
MAX(Loser_Pts)  AS "Lowest Loser Pts",
ROUND (AVG(Loser_Pts))  AS "Average Loser Pts", 
ROUND (AVG(Winner_pts))  AS "Average Winner Pts"
FROM superbowls;

-- How many times each MVP has been MVP --
SELECT MVP, COUNT(MVP) FROM superbowls
GROUP BY MVP
HAVING COUNT(MVP)
ORDER BY COUNT(MVP) DESC;

 -- Game performance based on Average of Winner points --
SELECT Winner, Winner_Pts,
CASE
	WHEN Winner_Pts > 30 THEN "Great Game"
    ELSE "Okay Game"
END AS "Game Performance"
FROM superbowls
ORDER BY Winner_Pts DESC;

-- Games in February & January --
SELECT * FROM superbowls
WHERE Date like '%Feb%';
...
-- Display all dead MVP --
SELECT Date, SB, MVP AS "Deceased MVPs"
FROM superbowls
WHERE MVP Like '%+';

-- Number of wins/loses by winning/losing teams --
SELECT Winner, COUNT(Winner) AS "Number of Wins"
FROM superbowls
GROUP BY Winner
ORDER BY COUNT(Winner) DESC ;

SELECT Loser, COUNT(Loser) AS "Number of Loses"
FROM superbowls
GROUP BY Loser
ORDER BY COUNT(Loser) DESC ;


SELECT Date, SB, Winner_Pts, Loser_Pts
FROM superbowls
WHERE (Winner_Pts - Loser_Pts) < 2;

-- which state has held the most superbowls --
SELECT 
	State,
	COUNT(State) AS number_times_hosted_SB
FROM superbowls
GROUP BY State
ORDER BY number_times_hosted_SB DESC;

SELECT State, COUNT(State) FROM superbowls
GROUP BY State;
